...
HRESULT InitPause(void)
{
	...
	// 画面暗転の中心座標
	g_posPauseBG.x = SCREEN_WIDTH / 2;
	g_posPauseBG.y = SCREEN_HEIGTH / 2;

	// ポーズ下地の中心座標
	g_posPause.x = SCREEN_WIDTH / 2;
	g_posPause.y = SCREEN_HEIGTH / 2;

	// Pauseの中心座標
	g_posPauseLogo.x = SCREEN_WIDTH / 2;
	g_posPauseLogo.y = 290;

	// Continueの中心座標
	g_posContinue.x = SCREEN_WIDTH / 2;
	g_posContinue.y = 420;

	// Retryの中心座標
	g_posRetry.x = SCREEN_WIDTH / 2;
	g_posRetry.y = 590;

	// Quitの中心座標
	g_posQuit.x = SCREEN_WIDTH / 2;
	g_posQuit.y = 760;
